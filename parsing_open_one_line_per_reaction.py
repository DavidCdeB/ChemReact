...
def table_open_reactions_one_line(Open_reactions):

    strings = [i[0][-1] for i in Open_reactions]
    dict_counter_strings = dict(Counter(strings))

    f = open('Reactions_Open_Table_one_line_per_reaction.tex', 'w')
...
\\hline
Reaction & Time  &  Indices & Max. dist. & Min. dist \\\ \\hline
...
#           if N == 1:
#           for k in range(0, len(all_interval)):
...
#               print (f"\\\ \cline{{3-6}}", file=f)
                print (f" {myreaction} & {all_interval[k]} & {all_word[k]} & {all_value_maxs_rounded[k]} & {all_value_mins_rounded[k]}", file=f)
